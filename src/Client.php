...
class Client
{
     ...
     * @throws ClientException Throw on API return invalid response.
     */
    public function postOrderRefund($orderId, $refundId, $amount, $receipt = '', $baseUrl = self::PAYMENTS_API_URI)
    {
        $url = $baseUrl . '/orders/' . $orderId . '/refunds/' . $refundId;

        if ($receipt) return $this->requestBuilder($url, self::POST, ['amount' => $amount, 'receipt' => $receipt]);
    ...
    protected function requestBuilder($url, $method = self::GET, array $body = [], $raw = false)
    {
        ...
        if (false === empty($response)) {
            ...
            if (null === $json) {
                //throw new ClientException($curl, "json decode error");
